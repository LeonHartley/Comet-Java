...
public class BackgroundTonerInteraction extends Interactor {
    @Override
    ...
    public boolean onInteract(int request, RoomItem item, GenericEntity avatar, boolean isWiredTriggered) {
        ...
        floorItem.getRoom().getEntities().broadcastMessage(UpdateFloorItemMessageComposer.compose(floorItem, item.getOwner()));
        return false;
    }
