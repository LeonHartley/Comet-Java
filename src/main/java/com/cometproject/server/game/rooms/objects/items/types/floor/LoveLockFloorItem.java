...
public class LoveLockFloorItem extends RoomItemFloor {
    ...
    public LoveLockFloorItem(int id, int itemId, Room room, int owner, int x, int y, double z, int rotation, String data) {
    ...
    public boolean onInteract(GenericEntity entity, int requestData, boolean isWiredTrigger) {
        ...
        try {
            ((PlayerEntity) leftEntity).getPlayer().getSession().send(LoveLockWidgetMessageComposer.compose(this.getId()));
            ((PlayerEntity) rightEntity).getPlayer().getSession().send(LoveLockWidgetMessageComposer.compose(this.getId()));
